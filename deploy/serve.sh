...
# Arguments: $1: model tag, $2: model version

set -e # Exit if any command fails

MODEL_PATH="output/ecdysis/$1/$2"
SERVERADDRESS="ecdysis01.local"

echo "Archiving model..."
torch-model-archiver --model-name "metaformer" --version "$2" --model-file "models/MetaFG.py" \
...
ERR_MSG=$(curl -s -X POST "$SERVERADDRESS:8075/models?url=metaformer.mar&initial_workers=1&synchronous=true" | jq -r '.message')
if [ "$ERR_MSG" != "null" ]
then
...
curl -X PUT -s "$SERVERADDRESS:8075/models/metaformer/$2?min_worker=16&max_worker=32" | jq -r .status
curl -X PUT -s "$SERVERADDRESS:8075/models/metaformer/$2/set-default" | jq -r .status
sleep 30

echo "Sending test request..."
TEST_ID=$(curl -s "$SERVERADDRESS:8074/predictions/metaformer" -T "tests/diabrotica.JPG" | jq -r .taxonid)
if [ "${TEST_ID}" = 1048497 ]
then
